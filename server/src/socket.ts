...
io.on('connection', socket => {
    ...
    socket.on('join-random-room', function() {
        try{
            let tempRoom: Room[] = rooms.filter(room => (room.password === undefined || room.password === '') && room.users.length < 6 && room.status != "endscreen" && (room.status == 'lobby' ? true : room.currentRound < room.maxRounds-4));
